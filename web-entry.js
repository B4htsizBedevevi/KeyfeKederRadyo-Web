...
function proxyToGateway(req, res) {
  ...
  proxy.setTimeout(20000, () => {
    proxy.destroy(new Error("Gateway timeout"));
  });
...
// API ve stream istekleri doğrudan gateway'e gider.
app.use("/api", proxyToGateway);

// Vite production build'i aynı public porttan sun.
...
function startGateway() {
  ...
  gateway.on("exit", (code, signal) => {
    ...
    if (!shuttingDown && code !== 0) {
      setTimeout(startGateway, 1500).unref();
    }
  });
}
...
const server = app.listen(PUBLIC_PORT, HOST, () => {
  ...
  log(`PUBLIC:   http://${HOST}:${PUBLIC_PORT}`);
  log(`GATEWAY:  http://127.0.0.1:${INTERNAL_PORT}`);
  log(`DIST:     ${DIST}`);
  log("================================================");
});
